...
KafkaConsumerFactory.prototype.createConsumers = function (consumersConfig) {
    ...
    names.forEach(function (name) {
        this.logger.debug('Consumer config found for ' + name);
        var func = consumersConfig[name].highLevel ? this.createHighLevelConsumer : this.createConsumer;
        consumers[name] = func(consumersConfig[name]);
    });
    return consumers;
};
